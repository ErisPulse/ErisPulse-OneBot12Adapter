...
- 标准兼容性：完全符合OneBot12标准，无需事件转换
- 旧配置兼容：兼容旧版本配置格式，提供迁移提醒
...
```python
{
    "type": "message",
    "detail_type": "private",
    "self": {"user_id": "main"},  // 发送事件的账户ID（标准字段）
    "platform": "onebot12",
    // ... 其他事件字段
}
```
...
1. **协议格式**: OneBot12是更现代的协议设计，事件格式更标准化
2. **消息处理**: 无需CQ码转换，直接使用消息段格式
3. **API接口**: 更统一的API设计，参数和返回值标准化
4. **事件字段**: 更简洁的事件结构，字段命名更规范
5. **扩展性**: 更好的平台扩展机制
